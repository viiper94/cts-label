                                ...
                                @foreach($courses_list as $courses)
                                    ...
                                    @foreach($courses as $course)
                                        <tr>
                                            ...
                                            <td>
                                                <a class='btn btn-success' href='{{ route('school_admin') }}/editcourse/{{ $course->id }}'>
                        ...
                        <table class="items-table table table-hover table__dark">
                            ...
                            @foreach($teachers_list as $teachers)
                                ...
                                @foreach($teachers as $teacher)
                                    <tr>
                                        <td><img src="/images/school/teachers/{{ $teacher->image }}" alt="{{ $teacher->name }}" class="teacher-img"></td>
                                        <td><h5>{{ $teacher->name }}</h5></td>
                                        <td><h5>{{ $teacher->lang }}</h5></td>
                                        <td>
                                            <a class='btn btn-success' href='{{ route('school_admin') }}/editteacher/{{ $teacher->id }}'>
                                                <span class='glyphicon glyphicon-pencil' aria-hidden='true'></span>
                                                <span class="hidden-xs hidden-sm">Редактировать</span>
                                            </a>
                                            <a class='btn btn-danger' href='{{ route('school_admin') }}/delete/{{ $teacher->id }}' onclick='return confirm("Удалить?")'>
                                                <span class='glyphicon glyphicon-trash' aria-hidden='true'></span>
                                                <span class="hidden-xs hidden-sm">Удалить</span>
                                            </a>
                                        </td>
                                        <td class="flex">
                                            <a class='btn btn-default btn-default__dark' href='{{ route('school_admin') }}/sort/{{ $teacher->id }}/up'>
                                                <span class='glyphicon glyphicon-arrow-up'></span>
                                            </a>
                                            <input type='number' class='form-control form-control__dark sort-input' name='sort[{{ $teacher->id }}]' value='{{ $teacher->sort_id }}'>
                                            <a class='btn btn-default btn-default__dark' href='{{ route('school_admin') }}/sort/{{ $teacher->id }}/down'>
                                                <span class='glyphicon glyphicon-arrow-down'></span>
                                            </a>
                                        </td>
                                    </tr>
